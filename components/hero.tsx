  ...
  return (
    <div ref={containerRef} className="relative min-h-screen bg-[#060608] text-white overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 opacity-40 pointer-events-none will-change-transform">
        <div className="absolute top-[-15%] left-[-15%] h-[50%] w-[50%] rounded-full bg-purple-900/30 blur-[120px]" />
        <div className="absolute bottom-[-15%] right-[-15%] h-[50%] w-[50%] rounded-full bg-yellow-600/20 blur-[120px]" />
      </div>
      ...
      >
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-black tracking-tighter bg-gradient-to-r from-white via-yellow-500 to-white bg-[length:200%_auto] animate-gradient-x bg-clip-text text-transparent">
            vikkymediatech
          </h1>
        </div>
        ...
        <nav className="hidden lg:flex items-center gap-8 xl:gap-12">
          {['HOME', 'PROJECTS', 'SERVICES', 'COURSES', 'CONTACT'].map((item, i) => (
            ...
            >
              <Link
                href={item === 'HOME' ? '/' : `#${item.toLowerCase()}`}
                className="relative text-sm font-bold tracking-wider text-white/70 hover:text-yellow-500 transition-colors duration-300 group"
              >
                {item}
                <span className="absolute -bottom-2 left-0 w-0 h-0.5 bg-yellow-500 transition-all duration-300 group-hover:w-full" />
              </Link>
            </motion.div>
          ))}
        </nav>
              ...
              {['HOME', 'PROJECTS', 'SERVICES', 'COURSES', 'CONTACT'].map((item, i) => (
                ...
                >
                  <Link
                    href={item === 'HOME' ? '/' : `#${item.toLowerCase()}`}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tighter hover:text-yellow-500 transition-colors duration-300"
                  >
                    {item}
                  </Link>
                </motion.div>
      ...
      <main className="relative grid grid-cols-1 lg:grid-cols-12 gap-8 items-end min-h-[calc(100vh-80px)] px-4 sm:px-6 md:px-12 lg:px-16 pt-12 sm:pt-16 lg:pt-8 pb-0">
        
        {/* LEFT — TEXT + SOCIALS (3 cols on desktop) */}
        <motion.div 
          style={{ y: leftY }} 
        ...
        >
          <div className="max-w-md mx-auto lg:mx-0 lg:max-w-sm">
            ...
            >
              ...
              <span className="text-yellow-500 text-[10px] sm:text-xs font-black tracking-[0.5em] sm:tracking-[0.6em]">
                INSTRUCTOR & DEV
              </span>
            </motion.div>
            ...
            >
              <Link 
                href="#projects" 
                className="inline-flex items-center gap-4 sm:gap-6 text-xs sm:text-sm font-bold uppercase tracking-widest text-yellow-500 hover:text-white transition-colors duration-300"
              >
                Explore Work
                <span className="h-12 w-12 sm:h-14 sm:w-14 rounded-full border-2 border-yellow-500 flex items-center justify-center hover:bg-yellow-500 hover:text-black transition-all duration-300">
                  →
                </span>
              </Link>
            </motion.div>
            ...
            >
              {socialLinks.map(({ icon: Icon, href }, i) => (
                <motion.a
                  ...
                  className="text-white/40 hover:text-yellow-500 transition-all duration-300"
                >
        ...
        {/* CENTER — MAIN IMAGE (6 cols on desktop) */}
        <div className="lg:col-span-6 relative flex justify-center items-end pb-0 order-first lg:order-none">
          {/* Controlled Yellow Circle */}
          <motion.div
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ duration: 1.5, delay: 0.2, type: 'spring', stiffness: 60, damping: 25 }}
            className="absolute -z-10 bottom-0 w-[300px] h-[300px] sm:w-[450px] sm:h-[450px] md:w-[600px] md:h-[600px] lg:w-[700px] lg:h-[700px] xl:w-[800px] xl:h-[800px] rounded-full bg-yellow-500/50 blur-[80px] sm:blur-[100px] lg:blur-3xl will-change-transform"
          />
          ...
          <motion.img
            ...
            loading="eager"
            className="relative z-20 w-full max-w-[350px] sm:max-w-[500px] md:max-w-[700px] lg:max-w-[900px] xl:max-w-[1100px] 2xl:max-w-[1300px] h-auto object-contain object-bottom select-none grayscale brightness-110 contrast-125 drop-shadow-[0_50px_100px_rgba(0,0,0,0.8)] will-change-transform"
            style={{ maxHeight: 'calc(100vh - 100px)' }}
          />
        </div>

        {/* RIGHT — DESIGN & CODE (3 cols on desktop) */}
        <motion.div 
          style={{ y: rightY }} 
        ...
        >
          ...
          <motion.p
            ...
            animate={{ opacity: 1 }}
            transition={{ duration: 0.8, delay: 0.7 }}
            className="text-[10px] sm:text-xs font-bold tracking-[0.4em] sm:tracking-[0.5em] text-white/30 uppercase"
          >
            Lalupon, Oyo State, Nigeria
          </motion.p>
        </motion.div>
